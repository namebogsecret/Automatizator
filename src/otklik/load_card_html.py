...
def wait_for_jquery_ajax(driver, max_delay_seconds=30):
    """
    Функция wait_for_jquery_ajax ожидает завершения запросов jQuery AJAX в течение указанного срока.
    
    :param driver: Параметр «driver» — это экземпляр WebDriver, который используется для взаимодействия
    с веб-браузером. Это может быть любая реализация WebDriver, например ChromeDriver от Selenium или
    GeckoDriver для Firefox. Этот параметр необходим для выполнения кода JavaScript в браузере и
    проверки статуса jQuery и AJAX
    :param max_delay_seconds: Параметр max_delay_секунды — это максимальное время (в секундах), в
    течение которого функция будет ожидать завершения jQuery AJAX. Если AJAX не будет завершен в течение
    этого времени, функция прекратит ожидание и продолжит выполнение остальной части кода, defaults to
    30 (optional)
    """
...
def load_card_html(url, driver, sql) -> tuple:
    """
    Функция load_card_html загружает HTML-страницу карточки, извлекает со страницы соответствующую
    информацию и выполняет действия на основе извлеченной информации.
    
    :param url: URL-адрес карты, которую необходимо загрузить
    :param driver: Параметр driver — это экземпляр веб-драйвера, такого как Selenium WebDriver, который
    используется для взаимодействия с веб-браузером. Он используется для перехода к указанному
    URL-адресу, щелчка по элементам и получения источника страницы
    :param sql: Параметр `sql`, скорее всего, представляет собой соединение с базой данных или
    SQL-запрос, используемый для взаимодействия с базой данных. Она не определена явно в предоставленном
    фрагменте кода, поэтому ее точное назначение и реализация будут зависеть от конкретного контекста, в
    котором используется эта функция
    :return: кортеж, содержащий следующие элементы:
    1. Строка, обозначающая статус процесса загрузки карты («Ошибка», «Всегда», «Неподходит», «Удалена»,
    «Ваканс», «Бан», «Лимит», «Отправлено»).
    2. Исходный HTML-код загруженной карты.
    3. Исходный HTML-код выбранных параметров ответа.
    4
    """
