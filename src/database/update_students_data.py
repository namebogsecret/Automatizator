...
def update_students_data(url:str, sql, status:str, html:str, html_choose:str, html_otklik_param:str, all_text:str):
    ...
    try:
        ...
        cursor.execute("INSERT INTO StudentsData (id, status, html, html_choose, html_otklik_param, all_text) VALUES (?, ?, ?, ?, ?, ?)", (iddb, status, html, html_choose, html_otklik_param, all_text))
        pik(100)
    except Exception as e:
        logger.error("Студента нет в базе данных. Error: %s", str(e))
        #cursor.execute("INSERT INTO StudentsData (iddb, status) VALUES (?, ?)", (iddb, status))
    sql.commit()
    return True